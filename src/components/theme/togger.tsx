...
export default function ThemeTogger() {
  const { setTheme, theme, forcedTheme, systemTheme } = useTheme()
  const [isHovering, setIsHovering] = useState(false);

  useEffect(() => {
    console.log(theme, forcedTheme, systemTheme)
  }, [theme]);
  return (
    <button
      aria-label="Toggle theme"
      className={`inline-flex ${isHovering
        ? "bg-gray-200 dark:bg-[#313131]"
        : ""
        }  active:bg-gray-300 transition-[background-color] dark:active:bg-[#242424] rounded-sm p-2 bg-gray-200 dark:bg-[#313131] theme-system:!bg-inherit [&_.sun-icon]:hidden dark:[&_.moon-icon]:hidden dark:[&_.sun-icon]:inline`}
    >
      <Sun />
    </button>
  )
