import { format, compareDesc, formatDistance, subDays } from "date-fns";
...
export function Article({ content, pages, title, time }: { content?: MDXRemoteSerializeResult, pages?: PageListItem[], title?: string, time?: Date }) {
  return (
    <>
      ...
      <div className="flex justify-between items-center mt-4 mb-6 text-sm max-w-[650px]">
        <span className="text-sm text-neutral-600 dark:text-neutral-400">{format(new Date(time as Date), "MMM dd, yyyy")} ({formatDistance(new Date(time as Date), new Date(), { addSuffix: true })})</span>
